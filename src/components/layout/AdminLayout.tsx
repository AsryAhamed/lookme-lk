...
export default function AdminLayout() {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      {/* Logo */}
      <div className="px-6 py-5 border-b border-sky-700/40 shrink-0">
        <span className="font-cormorant text-white text-xl font-semibold tracking-wide">
          look<span className="text-sky-300">me</span>.lk
        </span>
        <p className="text-xs text-sky-200/40 mt-0.5 uppercase tracking-wider">
          Admin Panel
        </p>
      </div>
      ...
      {/* Bottom links */}
      <div className="px-3 py-4 border-t border-sky-700/30 space-y-0.5 shrink-0">
        <a
          href="/"
          target="_blank"
      ...
      {adminName && (
        <div className="px-4 pb-4 shrink-0">
          <div className="bg-sky-500/10 rounded-sm px-3 py-2">
            ...
            <p className="text-xs text-sky-100/80 font-medium mt-0.5 truncate">
              {adminName}
            </p>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="md:hidden flex items-center justify-between px-4 py-3 bg-sky-950 border-b border-sky-700/30 shrink-0">
          <button
            ...
            className="text-sky-100/70 hover:text-white transition-colors"
          >
            <Menu size={22} />
          </button>
          ...
          <button
            onClick={handleSignOut}
            className="text-sky-100/40 hover:text-red-400 transition-colors"
          >
            <LogOut size={18} />
          </button>
        </header>
