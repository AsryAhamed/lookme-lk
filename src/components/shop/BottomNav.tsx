  ...
  return (
    <div
      ...
      <div className="flex items-center justify-around h-16">
        ...
        {/* Home */}
        <Link to="/"
          className={`flex flex-col items-center gap-0.5 px-4 py-2 transition-colors ${path === '/' ? 'text-gold' : 'text-muted'}`}>
          <Home size={22} strokeWidth={path === '/' ? 2 : 1.5} />
          <span className="text-[10px] uppercase tracking-wider">Home</span>
        </Link>

        {/* Shop */}
        <Link to="/shop"
          className={`flex flex-col items-center gap-0.5 px-4 py-2 transition-colors ${path.startsWith('/shop') ? 'text-gold' : 'text-muted'}`}>
          <Grid3x3 size={22} strokeWidth={path.startsWith('/shop') ? 2 : 1.5} />
          <span className="text-[10px] uppercase tracking-wider">Shop</span>
        </Link>

        {/* Cart */}
        <button onClick={toggleCart}
          className="flex flex-col items-center gap-0.5 px-4 py-2 text-muted">
        ...
        <a href="https://wa.me/94766604555" target="_blank" rel="noreferrer"
          ...
          <MessageCircle size={22} strokeWidth={1.5} className="text-[#25D366]" />
          <span className="text-[10px] uppercase tracking-wider text-[#25D366]">Order</span>
        </a>
